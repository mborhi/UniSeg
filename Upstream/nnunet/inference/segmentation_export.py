...
def save_softmax_nifti_from_softmax_gt_size(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
                                         properties_dict: dict, order: int = 1,
                                         region_class_order: Tuple[Tuple[int]] = None,
                                         seg_postprogess_fn: callable = None, seg_postprocess_args: tuple = None,
                                         resampled_npz_fname: str = None,
    ...
    print(f"Shape before crop: {shape_original_before_cropping}")
    print(f"Shape after crop: {shape_original_after_cropping}")
    print(f"Current shape: {current_shape}")
    print(f"Segmentation softmax: {segmentation_softmax.shape}")
    print(f"Segmentation softmax unique: {np.unique(segmentation_softmax)}")
    ...
    if bbox is not None:
        # seg_old_size = np.zeros(shape_original_before_cropping, dtype=np.uint8)
        # seg_old_size = np.zeros(new_shape, dtype=np.uint8)
        seg_old_size = np.zeros(new_shape, dtype=np.float16)
        for c in range(3):
            # bbox[c][1] = np.min((bbox[c][0] + seg_old_spacing.shape[c], shape_original_before_cropping[c+1]))
            bbox[c][1] = np.min((bbox[c][0] + seg_old_spacing.shape[c+1], new_shape[c+1]))
        seg_old_size[:, bbox[0][0]:bbox[0][1],
        bbox[1][0]:bbox[1][1],
        bbox[2][0]:bbox[2][1]] = seg_old_spacing
        # seg_old_size[:, bbox[1][0]:bbox[1][1],
        # bbox[2][0]:bbox[2][1],
        # bbox[3][0]:bbox[3][1]] = seg_old_spacing
    else:
        seg_old_size = seg_old_spacing
    ...
    print(f"Seg old size pre processed: {seg_old_size.shape}")
    print(f"Seg old size pre processed vals: {np.unique(seg_old_size)}")
...
def save_softmax_nifti_from_softmax_alt(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
