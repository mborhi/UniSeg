...
CUDA_VISIBLE_DEVICES=$GPU nnUNet_n_proc_DA=32 python3 -m torch.distributed.launch --master_port=4123 --nproc_per_node=1 run/run_training_DDP.py "3d_fullres" $Traniner 92 0 -exp_name $task_exp --dbs -loss_type "gnlll" -feature_space_dim 32 -update_iter 2 -max_num_epochs 3 -queue_size 5000 -batch_size 1 -num_gpus 1 #-val --valbest # --with_wandb -wandb_project_name $wandb_project_name

python3 UniSeg_Metrics_test.py --result_path $task_exp'/3d_fullres/Task092_QuickTest/'$Traniner'__DoDNetPlans/fold_0/validation_raw/'